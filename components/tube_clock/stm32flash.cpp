...
// ---------------------------------------------------------------------------
// Non-blocking protocol helpers
// ---------------------------------------------------------------------------

stm32_err_t stm32_poll_ack(const stm32_unique_ptr &stm) {
  auto *stream = stm->stream;
  if (!stream->available())
    return STM32_ERR_PENDING;
  uint8_t byte;
  ...
  if (byte == STM32_BUSY)
    return STM32_ERR_PENDING;
  ESP_LOGD(TAG, "stm32_poll_ack: unexpected byte 0x%02x", byte);
  return STM32_ERR_UNKNOWN;
}
...
stm32_err_t stm32_erase_page_send(const stm32_unique_ptr &stm, uint32_t page_num) {
  ...
  // Send command + complement; wait for command ACK (~1 ms blocking, acceptable)
  if (stm32_send_command(stm, stm->cmd->er) != STM32_ERR_OK)
    return STM32_ERR_UNKNOWN;

  if (stm->cmd->er == STM32_CMD_ER) {
    // Regular erase (0x43): [N-1, page, checksum], N=1
    const uint8_t pg = static_cast<uint8_t>(page_num);
    const uint8_t buf[3] = {0x00, pg, pg};  // 0x00 ^ pg = pg
    stream->write_array(buf, 3);
  } else {
    // Extended erase (0x44 / 0x45): 2-byte (N-1), 2-byte page number, checksum
    const uint8_t ph = static_cast<uint8_t>(page_num >> 8);
    const uint8_t pl = static_cast<uint8_t>(page_num & 0xFF);
    const uint8_t buf[5] = {0x00, 0x00, ph, pl, static_cast<uint8_t>(ph ^ pl)};
    stream->write_array(buf, 5);
  }
  stream->flush();
  // Final erase-complete ACK is deferred; caller polls stm32_poll_ack() with ≤5s deadline.
  return STM32_ERR_OK;
}
...
stm32_err_t stm32_write_memory_send(const stm32_unique_ptr &stm, uint32_t address, const uint8_t *data,
  ...
  if (len > 256) {
    ESP_LOGD(TAG, "Error: WRITE length limit at 256 bytes");
    return STM32_ERR_UNKNOWN;
  }
  ...
  // Send command + complement; wait for command ACK (~1 ms)
  if (stm32_send_command(stm, stm->cmd->wm) != STM32_ERR_OK)
    return STM32_ERR_UNKNOWN;

  // Send address + checksum; wait for address ACK (~1 ms)
  static constexpr auto ADDR_BUF_SIZE = 5;
  uint8_t addr_buf[ADDR_BUF_SIZE];
  populate_buffer_with_address(addr_buf, address);
  stream->write_array(addr_buf, ADDR_BUF_SIZE);
  stream->flush();
  if (stm32_get_ack(stm) != STM32_ERR_OK)
    return STM32_ERR_UNKNOWN;

  // Build and send data payload (4-byte aligned, 0xFF padding)
  const unsigned int aligned_len = (len + 3) & ~3u;
  uint8_t cs = static_cast<uint8_t>(aligned_len - 1);
  uint8_t buf[256 + 2];
  buf[0] = static_cast<uint8_t>(aligned_len - 1);
  for (unsigned int i = 0; i < len; i++) {
    cs ^= data[i];
    buf[i + 1] = data[i];
  }
  for (unsigned int i = len; i < aligned_len; i++) {
    cs ^= 0xFF;
    buf[i + 1] = 0xFF;
  }
  buf[aligned_len + 1] = cs;
  stream->write_array(buf, aligned_len + 2);
  stream->flush();
  // Final write-complete ACK is deferred; caller polls stm32_poll_ack() with ≤1s deadline.
  return STM32_ERR_OK;
}

// ---------------------------------------------------------------------------
// Stm32Flasher implementation
// ---------------------------------------------------------------------------

void Stm32Flasher::begin(uart::UARTDevice *stream, size_t total_bytes, uint32_t settings_page) {
  this->reset();
  this->stream_ = stream;
  this->total_bytes_ = total_bytes;
  this->settings_page_ = settings_page;
  this->bytes_remaining_ = total_bytes;
  this->state_ = FlashState::INIT_ATTEMPT;
}

Stm32Flasher::Result Stm32Flasher::tick() {
  switch (this->state_) {
    case FlashState::IDLE:
      return Result::PENDING;

    case FlashState::INIT_ATTEMPT: {
      App.feed_wdt();
      if (this->init_retry_ms_ != 0 && millis() < this->init_retry_ms_)
        return Result::PENDING;
      this->stm_ = stm32_init(this->stream_, STREAM_SERIAL, 1);
      if (!this->stm_) {
        this->init_attempts_++;
        if (this->init_attempts_ < 5) {
          this->init_retry_ms_ = millis() + 4;
          return Result::PENDING;
        }
        ESP_LOGE(TAG, "STM32 init failed after 5 attempts");
        this->state_ = FlashState::FAILED;
        return Result::FAILED;
      }
      const uint32_t page_size = this->stm_->dev->fl_ps[0];
      const uint32_t firmware_pages =
          (static_cast<uint32_t>(this->total_bytes_) + page_size - 1) / page_size;
      this->erase_pages_total_ = std::min(firmware_pages, this->settings_page_);
      this->erase_page_ = 0;
      this->write_addr_ = this->stm_->dev->fl_start;
      ESP_LOGD(TAG, "STM32 init OK, erasing %u pages (settings protected from page %u)",
               this->erase_pages_total_, this->settings_page_);
      this->state_ = FlashState::ERASE_PAGE_SEND;
      return Result::PENDING;
    }

    case FlashState::ERASE_PAGE_SEND: {
      App.feed_wdt();
      if (this->erase_page_ >= this->erase_pages_total_) {
        ESP_LOGD(TAG, "Erase complete, starting write");
        this->state_ = FlashState::WRITE_NEED_DATA;
        return Result::NEEDS_DATA;
      }
      ESP_LOGD(TAG, "Erasing page %u / %u", this->erase_page_ + 1, this->erase_pages_total_);
      if (stm32_erase_page_send(this->stm_, this->erase_page_) != STM32_ERR_OK) {
        ESP_LOGE(TAG, "Failed to send erase command for page %u", this->erase_page_);
        this->state_ = FlashState::FAILED;
        return Result::FAILED;
      }
      this->deadline_ms_ = millis() + STM32_PAGEERASE_TIMEOUT;
      this->state_ = FlashState::ERASE_PAGE_WAIT;
      return Result::PENDING;
    }

    case FlashState::ERASE_PAGE_WAIT: {
      App.feed_wdt();
      const auto err = stm32_poll_ack(this->stm_);
      if (err == STM32_ERR_PENDING) {
        if (millis() < this->deadline_ms_)
          return Result::PENDING;
        ESP_LOGE(TAG, "Erase ACK timeout for page %u", this->erase_page_);
        this->state_ = FlashState::FAILED;
        return Result::FAILED;
      }
      if (err != STM32_ERR_OK) {
        ESP_LOGE(TAG, "Erase NACK/error for page %u", this->erase_page_);
        this->state_ = FlashState::FAILED;
        return Result::FAILED;
      }
      this->erase_page_++;
    ...
    case FlashState::WRITE_NEED_DATA:
      return Result::NEEDS_DATA;

    case FlashState::WRITE_SEND: {
      App.feed_wdt();
      if (stm32_write_memory_send(this->stm_, this->write_addr_, this->chunk_buf_, this->chunk_len_) !=
          STM32_ERR_OK) {
        ESP_LOGE(TAG, "Failed to send write command at 0x%08x", this->write_addr_);
        this->state_ = FlashState::FAILED;
        return Result::FAILED;
      }
      this->deadline_ms_ = millis() + STM32_BLKWRITE_TIMEOUT;
      this->state_ = FlashState::WRITE_WAIT;
      return Result::PENDING;
    }

    case FlashState::WRITE_WAIT: {
      App.feed_wdt();
      const auto err = stm32_poll_ack(this->stm_);
      if (err == STM32_ERR_PENDING) {
        if (millis() < this->deadline_ms_)
          return Result::PENDING;
        ESP_LOGE(TAG, "Write ACK timeout at 0x%08x", this->write_addr_);
        this->state_ = FlashState::FAILED;
        return Result::FAILED;
      }
      if (err != STM32_ERR_OK) {
        ESP_LOGE(TAG, "Write NACK/error at 0x%08x", this->write_addr_);
        this->state_ = FlashState::FAILED;
        return Result::FAILED;
      }
      const unsigned int aligned_len = (this->chunk_len_ + 3) & ~3u;
      this->write_addr_ += aligned_len;
      this->bytes_remaining_ -= std::min(this->chunk_len_, this->bytes_remaining_);
      this->progress_ =
          1.0f - (static_cast<float>(this->bytes_remaining_) / static_cast<float>(this->total_bytes_));
      this->chunk_len_ = 0;
      if (this->bytes_remaining_ == 0) {
        this->state_ = FlashState::FINALIZE;
        return Result::PENDING;
      }
      this->state_ = FlashState::WRITE_NEED_DATA;
      return Result::NEEDS_DATA;
    ...
    case FlashState::FINALIZE: {
      App.feed_wdt();
      if (stm32_go(this->stm_, this->stm_->dev->fl_start) != STM32_ERR_OK)
        ESP_LOGW(TAG, "GO command failed (non-fatal), firmware should still boot");
      this->state_ = FlashState::DONE;
      return Result::DONE;
    }
...
void Stm32Flasher::provide_chunk(const uint8_t *data, unsigned int len) {
  if (this->state_ != FlashState::WRITE_NEED_DATA)
    return;
  const unsigned int copy_len = std::min(len, static_cast<unsigned int>(sizeof(this->chunk_buf_)));
  memcpy(this->chunk_buf_, data, copy_len);
  this->chunk_len_ = copy_len;
  this->state_ = FlashState::WRITE_SEND;
}

void Stm32Flasher::reset() {
  this->stm_ = stm32_unique_ptr{nullptr, nullptr};
  this->state_ = FlashState::IDLE;
  this->deadline_ms_ = 0;
  this->erase_page_ = 0;
  this->erase_pages_total_ = 0;
  this->settings_page_ = 0;
  this->total_bytes_ = 0;
  this->write_addr_ = 0;
