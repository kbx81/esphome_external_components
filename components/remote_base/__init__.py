...
# Honeywell String Lights Protocol
HSLData, HSLBinarySensor, HSLTrigger, HSLAction, HSLDumper = declare_protocol("HSL")
HSL_SCHEMA = cv.Schema({cv.Required(CONF_DATA): cv.hex_uint32_t})


@register_binary_sensor("honeywell_string_lights", HSLBinarySensor, HSL_SCHEMA)
def hsl_binary_sensor(var, config):
    cg.add(
        var.set_data(
            cg.StructInitializer(
                HSLData,
...
@register_trigger("honeywell_string_lights", HSLTrigger, HSLData)
def hsl_trigger(var, config):
    pass


@register_dumper("honeywell_string_lights", HSLDumper)
def hsl_dumper(var, config):
    pass


@register_action("honeywell_string_lights", HSLAction, HSL_SCHEMA)
async def hsl_action(var, config, args):
    template_ = await cg.templatable(config[CONF_DATA], args, cg.uint32)
    cg.add(var.set_data(template_))
